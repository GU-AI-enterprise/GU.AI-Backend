...
CREATE TABLE public.ai_jobs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  type character varying NOT NULL CHECK (type::text = ANY (ARRAY['try_on'::text, 'try_on_max'::text, 'generate'::text, 'edit'::text, 'remove_bg'::text, 'upscale'::text, 'product_to_model'::text, 'reframe'::text, 'face_swap'::text, 'model_swap'::text, 'create_model'::text, 'image_to_video'::text])),
  ...
  provider character varying NOT NULL CHECK (provider::text = ANY (ARRAY['nano_banana'::character varying, 'remove_bg'::character varying, 'fashn'::character varying]::text[])),
